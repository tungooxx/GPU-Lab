    ...
    def compile(self, policy: dict[str, Any], target_provider: str = "GENERIC", target_runtime: str = "default") -> dict[str, Any]:
        ...
        lines = [
            ...
            *[f"- {item.replace('_', ' ')}." for item in invariants],
            "Cross-project strategy-transfer policy:",
            "- Project scientific evidence never transfers automatically; a StrategyPattern is a candidate method, not target evidence.",
            "- Retrieve only structurally relevant strategies, screen applicability before use, and record retrieved, considered, and applied separately.",
            "- Negative transfer is valuable evidence; invalid implementation is not negative transfer.",
            "- Do not promote a strategy to GLOBAL merely because it worked a few times; scope promotion is an explicit reversible decision.",
            "- During independent discovery, preserve STATE_ONLY_GENERATION workers so strategy memory cannot homogenize candidate generation.",
            "Operating policy:",
