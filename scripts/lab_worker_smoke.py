...
def main() -> None:
    ...
    work_a = lab.create_work(project_id, "LITERATURE", "Independent literature", "Find evidence", "LITERATURE_RESEARCHER", first["worker"]["id"], created_session_id=first["session_id"])
    work_b = lab.create_work(project_id, "REVIEW", "Independent review", "Review a branch", "ADVERSARIAL_REVIEWER", second["worker"]["id"], created_session_id=second["session_id"])
    lab.claim_work(work_a["id"], first["worker"]["id"], first["session_id"])
    lab.claim_work(work_b["id"], second["worker"]["id"], second["session_id"])
    lab.message_send(project_id, first["worker"]["id"], first["session_id"], "REQUEST_REVIEW", "Review", "Please review the branch.", to_worker_id=second["worker"]["id"])
    run = store.object_create(project_id, "ExperimentRun", {"mode": "smoke"}, "EXPERIMENT_STARTED", "running")
    inspection = lab.create_work(project_id, "INSPECT_RESULT", "Inspect E1", "Wait for E1", "RESULT_INSPECTOR", first["worker"]["id"], dependencies=[{"target_type": "EXPERIMENT_RUN", "target_id": run["id"], "required_statuses": ["completed"]}], created_session_id=first["session_id"])
